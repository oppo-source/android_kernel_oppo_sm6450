...
static struct scsi_device *ufsringbuf_get_sdev(struct ufsf_feature *ufsf,
					int target_lun)
{
...
{
	return ((u64)be32_to_cpu(desc->host_time_a) << 32 |
		be32_to_cpu(desc->host_time_b));
}
				      ...
				      struct utp_upiu_req *upiu_req)
{
	...
	count += snprintf(buf + count, 21, "dev_time=%-10u ",
			  be32_to_cpu(desc->device_time));

	count += snprintf(buf + count, 10, "lun=0x%-2x ",
			  (be32_to_cpu(upiu_req->header.dword_0) >> 8) & 0xFF);

	count += snprintf(buf + count, 8, "tag=%-2d ",
			  be32_to_cpu(upiu_req->header.dword_0) & 0xFF);
	...
	if ((opcode == READ_10) || (opcode == WRITE_10)) {
		count += snprintf(buf + count, 16, "lba=0x%.8x ",
				  get_unaligned_be32(upiu_req->sc.cdb + 2));

		count +=
			snprintf(buf + count, 18, "txfer_len=%-6d ",
				 get_unaligned_be32(
					&upiu_req->sc.exp_data_transfer_len));
	} else {
				      ...
				      struct utp_upiu_req *upiu_req)
{
	...
	count += snprintf(buf + count, 10, "lun=0x%-2x ",
			  (be32_to_cpu(upiu_task_req->header.dword_0) >> 8)
			  & 0xFF);

	count += snprintf(buf + count, 8, "tag=%-2d ",
			  be32_to_cpu(upiu_task_req->header.dword_0) & 0xFF);

	count += snprintf(buf + count, 13, "cmd_id=0x%.2x ",
			  (be32_to_cpu(upiu_task_req->header.dword_1) >> 24)
			  & 0xFF);
					  ...
					  struct history_block_desc *desc)
{
	...
	u8 command = (be32_to_cpu(upiu_req->header.dword_0) >> 24) & 0xFF;
				...
				struct seq_file *file, void *buf)
{
	...
	data_length = get_unaligned_be24(buf);
	if (data_length == 0)
...
/*
 * scsi_execute_cmd() will copy cdb by 10-byte due to opcode.
...
static int ufsringbuf_issue_vendor_mode(struct ufsf_feature *ufsf, u8 code)
{
	...
	ret = scsi_execute_cmd(sdev, cdb, REQ_OP_DRV_IN, NULL, 0,
			       VENDOR_CMD_TIMEOUT, 0, &args);
					...
					void *buf, int transfer_bytes)
{
	...
	put_unaligned_be24(transfer_bytes, cdb + 6);

	for (retries = 0; retries < 3; retries++) {
		ret = scsi_execute_cmd(sdev, cdb, REQ_OP_DRV_IN, buf,
				       transfer_bytes, msecs_to_jiffies(30000),
				       3, &args);
		if (ret)
...
void ufsringbuf_get_dev_info(struct ufsf_feature *ufsf, u8 *desc_buf)
{
	...
	spec_version =
		get_unaligned_be16(desc_buf + DEVICE_DESC_PARAM_RING_BUF_VER);
...
void ufsringbuf_get_geo_info(struct ufsf_feature *ufsf, u8 *geo_buf)
{
	...
	hist_buf_size =	get_unaligned_be16(
			geo_buf + GEOMETRY_DESC_RINGBUF_MAX_HIST_BUFSIZE);
	if (!hist_buf_size) {
	...
	INFO_MSG("[FD:FE] wMaxCommandHistoryBufferSize %u", hist_buf_size);
...
static int ufsringbuf_proc_print_open(struct inode *inode, struct file *file)
{
	...
	return ret ? ret : single_open(file, ufsringbuf_proc_print_show,
				       pde_data(inode));
}
...
static int __set_record_en(struct ufsf_feature *ufsf, bool set)
{
	...
	ret = ufshcd_query_flag_retry(ufsf->hba, op,
				      QUERY_FLAG_IDN_CMD_HISTORY_RECORD_EN, 0,
				      NULL);
	if (ret) {
		ERR_MSG("query fail. (%d)", ret);
