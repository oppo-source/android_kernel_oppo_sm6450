...
#define HID(name) YMTC_HID_##name
...
enum UFSHID_DEV_STATE_YMTC {
	HID(IDLE)			= 0x0,
	HID(IN_PROGRESS)		= 0x1,
	HID(STOPPED_PREMATURELY)		= 0x2,
	HID(COMPLETED_SUCCESSFULLY)	= 0x3,
	HID(FAILED_BUSY)			= 0x4,
	HID(GENERAL_FAIL)		= 0x5,
	HID(NUM_DEV_STATES)		= 0x6
};
...
static int ufshid_ymtc_get_dev_info(struct ufshid_ymtc_info *hid_info)
{
	...
	INFO_MSG("bUFSExFeaturesSupport:YMTC HID support(no check)");
	...
	return ret;
err_out:
	ufshid_ymtc_set_state(hid_info, HID(FAILED));
	return ret;
}

static inline int ufshid_issue_disable(struct ufshid_ymtc_dev *hid)
{
	u8 idn = QUERY_FLAG_IDN_HID_ENABLE;
	int ret = 0;
...
static ssize_t ufshid_sysfs_show_suggested_hid_freq(struct ufshid_ymtc_dev *hid, char *buf)
{
	...
	INFO_MSG("suggested_hid_freq %u", attr_val);
...
static int ufshid_hold_runtime_pm(struct ufshid_ymtc_dev *hid)
{
	...
	if (ufshid_ymtc_get_state(hid->hid_info) == HID(SUSPEND) &&
...
static void ufshid_trigger_work_fn(struct work_struct *dwork)
{
	...
	if (ret)
		WARN_MSG("read HID State fail.. (err %d)", ret);
...
void ufshid_ymtc_reset_host(struct ufshid_ymtc_info *hid_info)
{
	...
	ufshid_ymtc_set_state(hid_info, HID(RESET));
...
void ufshid_ymtc_reset(struct ufshid_ymtc_info *hid_info)
{
	...
	ufshid_ymtc_set_state(hid_info, HID(PRESENT));
...
inline void ufsf_yhid_reset_host(struct ufshid_ymtc_info *hid_info)
{
	...
	if (ufshid_ymtc_get_state(hid_info) == HID(PRESENT))
...
static void ufshid_resume_work_handler(struct work_struct *work)
{
	...
	if (ufshcd_is_ufs_dev_active(hba) && ufshcd_is_link_active(hba))
		if (ufshid_ymtc_get_state(hid_info) == HID(SUSPEND))
...
static void ufshid_device_check_work_handler(struct work_struct *work)
{
	...
	if (ufshid_ymtc_get_state(hid_info) == HID(NEED_INIT))
...
inline void ufsf_yhid_set_init_state(struct ufs_hba *hba)
{
	...
	ufshid_ymtc_set_state(hid_info, HID(NEED_INIT));
}
...
out:
	ufshid_ymtc_set_state(hid_info, HID(SUSPEND));
...
static ssize_t ufshid_sysfs_show_version(struct ufshid_ymtc_dev *hid, char *buf)
{
	...
	INFO_MSG("YMTC HID version (%.4X)", UFSHID_VER_YMTC);
...
static ssize_t ufshid_sysfs_store_enable(struct ufshid_ymtc_dev *hid,
...
static ssize_t ufshid_sysfs_show_hid_state(struct ufshid_ymtc_dev *hid, char *buf)
{
	...
	if (attr_val >= HID(NUM_DEV_STATES))
		return -EINVAL;
...
kobj_del:
	...
	INFO_MSG("MID:0x%x, kobject removed (%d)", UFS_VENDOR_YMTC, err);
	kobject_del(&hid->kobj);
