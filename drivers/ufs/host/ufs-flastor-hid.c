...
#define HID(name) FLASTOR_HID_##name
...
#undef UFSHCD_EH_IN_PROGRESS
#define	UFSHCD_EH_IN_PROGRESS  (1 << 0)
...
static int ufshid_set_flag(struct ufshid_flastor_dev *hid, u8 idn)
{
	...
	HID_DEBUG(hid, "hid_flag set [0x%.2X] %u", idn, flag_res);
err_out:
	ufs_fhid_rpm_put_noidle(hba);
...
static int ufshid_clear_flag(struct ufshid_flastor_dev *hid, u8 idn)
{
	...
	HID_DEBUG(hid, "hid_flag clear [0x%.2X] %u", idn, flag_res);
err_out:
	ufs_fhid_rpm_put_noidle(hba);
...
static int ufshid_flastor_get_dev_info(struct ufshid_flastor_info *hid_info)
{
	...
	unsigned int support_bit = 0;
	struct ufs_hba *hba = hid_info->hba;

	ret = ufs_read_desc(hba, QUERY_DESC_IDN_DEVICE, 0, desc_buf,
	...
	if (hba->dev_info.wspecversion == 0x0220)
		support_bit = UFS_FEATURE_SUPPORT_HID_BIT_F_220;
	else if (hba->dev_info.wspecversion == 0x0310)
		support_bit = UFS_FEATURE_SUPPORT_HID_BIT_F_310;
	else {
		ERR_MSG("UFS version:0x%x not support ufshid", hba->dev_info.wspecversion);
		goto err_out;
	}

	if (!(get_unaligned_be32(desc_buf + DEVICE_DESC_PARAM_EX_FEAT_SUP) & support_bit)) {
		INFO_MSG("bUFSExFeaturesSupport: F HID not support");
		ret = -EOPNOTSUPP;
		goto err_out;
	}

	INFO_MSG("bUFSExFeaturesSupport:FLASTOR HID support");
	...
	return ret;
err_out:
	ufshid_flastor_set_state(hid_info, HID(FAILED));
	return ret;
}
...
static int ufshid_get_attr_frag_level(struct ufshid_flastor_dev *hid, u32 *attr_val)
{
   ...
   if (!ret)
	   HID_DEBUG(hid, "Frag_lv %d", *attr_val & HID_FRAG_LEVEL_MASK);
...
static int ufshid_get_attr_progress_ratio(struct ufshid_flastor_dev *hid, u32 *attr_val)
{
   ...
   if (!ret)
	   HID_DEBUG(hid, "progress_ratio %d", *attr_val);
...
static u32 ufs_pending_cmds(struct ufs_hba *hba)
{
...
int ufs_fhid_wait_for_doorbell_clr(struct ufs_hba *hba, u64 wait_timeout_us)
{
	...
	do {
		...
		tr_pending = ufs_pending_cmds(hba);
...
static int ufshid_hold_runtime_pm(struct ufshid_flastor_dev *hid)
{
	...
	if (ufshid_flastor_get_state(hid->hid_info) == HID(SUSPEND) &&
...
static void ufshid_trigger_work_fn(struct work_struct *dwork)
{
	...
	if (!ret && !ufshid_check_progress_end(attr_val)) {
		HID_DEBUG(hid, "HID is on-going(state:%d), so re-sched (%d ms)",
	...
	if (ret)
		WARN_MSG("get attr fragmented level fail.. (err %d)", ret);

	ret = ufshid_trigger_off(hid);
	if (ret)
		WARN_MSG("trigger off fail.. must check it (err %d)", ret);
...
void ufshid_flastor_reset_host(struct ufshid_flastor_info *hid_info)
{
	...
	ufshid_flastor_set_state(hid_info, HID(RESET));
...
void ufshid_flastor_reset(struct ufshid_flastor_info *hid_info)
{
	...
	ufshid_flastor_set_state(hid_info, HID(PRESENT));
...
inline void ufsf_fhid_reset_host(struct ufshid_flastor_info *hid_info)
{
	...
	if (ufshid_flastor_get_state(hid_info) == HID(PRESENT))
...
static void ufshid_resume_work_handler(struct work_struct *work)
{
	...
	if (ufshcd_is_ufs_dev_active(hba) && ufshcd_is_link_active(hba))
		if (ufshid_flastor_get_state(hid_info) == HID(SUSPEND))
...
static void ufshid_device_check_work_handler(struct work_struct *work)
{
	...
	if (ufshid_flastor_get_state(hid_info) == HID(NEED_INIT))
...
inline void ufsf_fhid_set_init_state(struct ufs_hba *hba)
{
	...
	if (ret) {
		ERR_MSG("register_trace_android_vh_ufs_update_sdev failed! ret = %d.", ret);
		return;
	}
	hid_info->hba = hba;
	...
	ufshid_flastor_set_state(hid_info, HID(NEED_INIT));
}
...
out:
	ufshid_flastor_set_state(hid_info, HID(SUSPEND));
...
static ssize_t ufshid_sysfs_show_version(struct ufshid_flastor_dev *hid, char *buf)
{
	...
	INFO_MSG("FLASTOR HID version (%.4X)", UFSHID_VER_FLASTOR);
...
{
	INFO_MSG("hid_enable %d", hid->hid_enable);

	return snprintf(buf, PAGE_SIZE, "%d\n", hid->hid_enable);
}

static ssize_t ufshid_sysfs_store_enable(struct ufshid_flastor_dev *hid,
					  const char *buf, size_t count)
{
	...
	INFO_MSG("%s HID Feature", val ? "Enable" : "Disable");
					  ...
					  const char *buf, size_t count)
{
	...
	if (!hid->hid_enable) {
		INFO_MSG("trigger fail as HID is disabled");
		return -EOPNOTSUPP;
	}
...
static ssize_t ufshid_sysfs_show_color(struct ufshid_flastor_dev *hid, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "%s\n", frag_level == HID(LEV_RED) ? "RED" :
		  frag_level == HID(LEV_YELLOW) ? "YELLOW" :
		  frag_level == HID(LEV_GREEN) ? "GREEN" :
		  frag_level == HID(LEV_GRAY) ? "GRAY" : "UNKNOWN");
...
static int ufshid_create_sysfs(struct ufshid_flastor_dev *hid)
{
	...
	INFO_MSG("MID:0x%x, ufshid creates sysfs ufshid %p dev->kobj %p",
	...
	if (!err) {
		for (entry = hid->sysfs_entries; entry->attr.name != NULL;
			 entry++) {
			INFO_MSG("MID:0x%x, ufshid sysfs attr creates: %s",
...
kobj_del:
	...
	INFO_MSG("MID:0x%x, kobject removed (%d)", UFS_VENDOR_FLASTOR, err);
	kobject_del(&hid->kobj);
