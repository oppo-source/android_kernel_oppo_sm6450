...
static int checkNfcChip(struct device *dev)
{
    ...
    for (i = 0; i < id_count; i++)
    {
        ...
        {
            if (strstr("THN31S|THN31F|THN31F-A", id_entries[i].chipset) == NULL)
            {
            ...
            pr_err("%s this nfc chipset:%s corresponds to this nfc driver", __func__, id_entries[i].chipset);
            found = true;
            break;
        }
    }
...
#endif
{
	...
	if (ret) {
		pr_err("%s: failed to checkNfcChip\n", __func__);
		return -ERROR; 
	}
