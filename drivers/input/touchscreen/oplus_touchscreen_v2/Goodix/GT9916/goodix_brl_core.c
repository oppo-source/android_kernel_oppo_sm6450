...
static enum hrtimer_restart fpga_check_hrtimer_handler(struct hrtimer *timer)
{
	if (!g_chip_info) {
		TPD_INFO("GT_brlD:%s, g_chip_info not exist!!\n", __func__);
		return 0;
	}

	TPD_INFO("GT_brlD:%s,hrtimer check:%d\n", __func__, g_chip_info->fpga_reset_check);
	if (g_chip_info->fpga_reset_check) {
		schedule_work(&g_chip_info->ts->fw_update_work);
	} else {
		TPD_INFO("GT_brlD:%s,not to updateFw\n", __func__);
	}
...
static int oplus_gt9916_fpga_state_change(struct notifier_block *nb, unsigned long event, void *v)
{
	...
	if (!g_chip_info->probe_complete) {
		TPD_INFO("GT_brlD:%s, probe not completed\n", __func__);
		return 0;
	}
	...
	TPD_INFO("GT_brlD:%s, fpga event:%lu start\n", __func__, event);
	ret = hrtimer_try_to_cancel(&g_chip_info->fpga_check_hrtimer);
	TPD_INFO("GT_brlD:%s,cancelHrtimer:%d\n", __func__, ret);
	/**
	 * ret = 1 The timer is active (started but not yet expired)
	 * ret = 0 The timer has expired and is executing a callback function
	 */
	msleep(5);
	hrtimer_start(&g_chip_info->fpga_check_hrtimer, g_chip_info->fpga_check_time, HRTIMER_MODE_REL);
	if (event == FPGA_RESET_PASS) {
		g_chip_info->fpga_reset_cnt++;
		TPD_INFO("GT_brlD:%s,FPGA_RESET_PASS[start hrtimer][cnt:%lu]\n", __func__, g_chip_info->fpga_reset_cnt);
		g_chip_info->fpga_reset_check = true;
	} else {
		g_chip_info->fpga_reset_check = false;
	}

	TPD_INFO("GT_brlD:%s, fpga event:%lu end\n", __func__, event);
	return 0;
}
#endif
...
static int goodix_gt9916_ts_probe(struct spi_device *spi)
{
	...
	TPD_INFO("GT_brlD:%s, start fpga hrtimer\n", __func__);
	hrtimer_init(&chip_info->fpga_check_hrtimer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	chip_info->fpga_check_time = ktime_set(GOODIX_CHECK_HRTIMER_S, GOODIX_CHECK_HRTIMER_NS);
	chip_info->fpga_check_hrtimer.function = fpga_check_hrtimer_handler;
	chip_info->fpga_reset_check = false;
	chip_info->fpga_reset_cnt = 0;
#endif
