...
static int oplus_gt9966_fpga_state_change(struct notifier_block *nb, unsigned long event, void *v)
{
	...
	if (!g_chip_info->probe_complete) {
		TPD_INFO("GT:%s, probe not completed\n", __func__);
		return 0;
	}
	...
	TPD_INFO("GT:%s, fpga event:%lu start\n", __func__, event);
	ret = hrtimer_try_to_cancel(&g_chip_info->fpga_check_hrtimer);
	TPD_INFO("GT:%s,cancelHrtimer:%d\n", __func__, ret);
	/**
...
static int goodix_gt9966_ts_probe(struct spi_device *spi)
{
	...
	TPD_INFO("GT:%s, start fpga hrtimer\n", __func__);
